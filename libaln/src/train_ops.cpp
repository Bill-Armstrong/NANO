...
CDataFile TVfile; // Training data, loaded into the training buffer adblTRdata
long nRowsTV; // the number of rows in the current training set in TVfile
...
// ALN pointers
CMyAln* pBaseNeuron = NULL; // declares a pointer to an ALN.

// Some global variables
int nDim; // Determined buy the analysis of the input file.
// double dblLimit;This is now in ALNDATAINFO as dblMSEorF, which is either the level of MSE where splitting stops or, if < 0, calls upon an F-test.
...
int nNotifyMask = AN_TRAIN | AN_EPOCH | AN_VECTORINFO; // Used with callbacks at different times for reporting on learning progress and controlling training.
...
void ALNAPI approximate() // routine
{
	...
	int nMaxEpochs = 20; // The number of passes through the data without splitting LFNs.
	double dblMinRMSE = 1e-20; // Stops training when the error is tiny.
	double dblLearnRate = 0.2;
	bStopTraining = FALSE; // Set to TRUE at the start of each epoch in alntrain.cpp. 
	// Set FALSE by any piece needing more training. 
	int nNumberLFNs = 1;  // initialize at 1
	// Tell the training algorithm the way to access the data using fillvector
	double dblLimit = -1; // Negative to split leaf nodes according to an F test;
