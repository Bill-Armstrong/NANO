                     ...
                     float fltResponse, BOOL bUsefulAdapt, const TRAINDATA* ptdata)
{
  ...
  // We have to think of the error as that of the *surface*, not the data point!
	...
	// float fltLearnRespParam = fltLearnRate * region.fltLearnFactor / (double)(2 * nDim - 1);
	float fltLearnRespParam = fltLearnRate / (double)(2 * nDim - 1);
	...
	float fltXmC = 0;
	// float Checktemp;  // temporary for checking
	float fltBend = 0;
	for (int i = 0; i < nDim -1; i++) //Skip the output centroid at nDim - 1.
	{
		...
		afltC[i] += fltXmC * fltLearnRespParam; // old centroid -> new centroid -> X
		// Update the distance of X[i] from the centroid (the compiler will optimize these steps)
		fltXmC = afltX[i] - afltC[i]; // should we do this???  MYTEST
		// ADAPT WEIGHTS
		...
		// It is NOT a uniform distribution in higher dimensions.  So we can't use this constant.
		...
		{
			fltBend = (fltXmC * fltXmC - afltD[i] > 0) ? -(fltXmC * fltXmC - afltD[i]) * fltError : (fltXmC * fltXmC - afltD[i]) * fltError; // Recall that the error is of the LFN value w.r.t. the sample value
