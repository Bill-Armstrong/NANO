...
int main()
{
	...
	double adblW[] = { 52317.8305,
						...
						-234.23716,
						-1.0 };
	// Note the first weight is the bias weight, used when the centroid of the linear piece is not used
	int nTRmaxSamples = file.RowCount();
	int nTRcols = nDim * 2 + 1;
	double dblMSEorF = -1.0;
	...
	pdata->nTRinsert = 0;
	pdata->dblMSEorF = dblMSEorF = 0.00001;
	...
	cout << "\nnTRmaxSamples = " << pdata->nTRmaxSamples << "  "
		...
		<< "\nnTRinsert = " << pdata->nTRinsert << endl;
	// Print the buffer contents
	for (int ii = 0; ii < 11; ii++)
	{
		for (int jj = 0; jj < 11; jj++)
		{
			cout << pdata->adblTRdata[ii * 11 + jj] << " ";
			if (jj == 4) cout << "closest = ";
		}
		cout << endl;
	}
	...
	for(int ntest = 0; ntest < 2; ntest++)
	{
		for (int i = 0; i < 6; i++)
		{
			LFN_W(pLFN)[i] = (ntest==0?adblW[i]:0); // set the weights of the single linear piece (bias weight at index 0)
		}
		// do analysis on both passes, but train only on second pass when ntest = 1
		if(ntest == 1) // train the ALN only in case ntest = 1
		{
			BOOL bJitter = FALSE;
			...
			for (int ii = 0; ii < 2; ii++)
			{
				...
				// bStop Training is not used because there is no splitting
			}
			cout << "Training succeeded!" << endl;
			for (int k = 0; k < nDim + 1; k++)
			{
				cout << LFN_W(pLFN)[k] << endl;
			}
		}
		// perform lfn analysis
		void* pvAnalysis = NULL;
		int nLFNStats = 0;
		pALN->LFNAnalysis(pvAnalysis,nLFNStats,AN_NONE,NULL,NULL);
		...
		for (int i = 0; i < nLFNStats; i++)
		{
			...
			int nWeightStats = 0;
			ALNNODE* pLFN = NULL;
			pALN->LFNStats(pvAnalysis, i, LFNStats, nWeightStats, pLFN);
			...
			for (int j = 0; j < nWeightStats-1; j++)
			{
				...
				pALN->LFNWeightStats(pvAnalysis, i, j, LFNWeightStats);
			...
			// free analysis data 
			pALN->LFNFreeAnalysis(pvAnalysis);
		}
